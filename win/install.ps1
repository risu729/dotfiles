...
function Invoke-ElevatedScript {
	...
	$command = "try { Invoke-RestMethod $($winScriptUrl.ToString()) | Invoke-Expression } catch { Write-Host `$PSItem } finally { Write-Host `$PSItem; Read-Host -Prompt 'Press Enter to exit' }"
	Write-Host $command
