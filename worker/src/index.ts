...
app.use(async (c, next) => {
	if (!c.env.REPOSITORY) {
		throw new HTTPException(500, {
			message: "Environment variable REPOSITORY is not set.",
		});
	}
	await next();
});
...
app.get("/", (c) => {
	return c.redirect(`https://github.com/${c.env.REPOSITORY}#readme`, 307);
});
...
app.get("/:os{win|wsl}", (c) => {
	...
	return c.redirect(
		`https://raw.githubusercontent.com/${c.env.REPOSITORY}/main/${os}/install.${
			os === "win" ? "ps1" : "sh"
		}`,
		307,
	);
