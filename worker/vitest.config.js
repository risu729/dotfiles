...
const currentBranch = process.env["CURRENT_BRANCH"] ?? execSync("git branch --show-current").toString().trim();
if (!currentBranch) {
...
export default defineConfig((configEnv) => mergeConfig(viteConfig(configEnv), defineWorkersConfig({
    test: {
        env: {
